...
bond_decoder_m = {(1): Chem.rdchem.BondType.SINGLE, (2): Chem.rdchem.
    BondType.DOUBLE, (3): Chem.rdchem.BondType.TRIPLE}
ATOM_VALENCY = {(6): 4, (7): 3, (8): 2, (9): 1, (15): 3, (16): 2, (17): 1,
    (35): 1, (53): 1}


def flatten_graph_data(adj, x):
    return paddle.concat(x=(adj.reshape([tuple(adj.shape)[0], -1]), x.
        reshape([tuple(x.shape)[0], -1])), axis=1)
...
def get_graph_data(x, num_nodes, num_relations, num_features):
    ...
    :return:
    """
    adj = x[:, :num_nodes * num_nodes * num_relations].reshape([-1,
        num_relations, num_nodes, num_nodes])
    feat_mat = x[:, num_nodes * num_nodes * num_relations:].reshape([-1,
...
def valid_mol(x):
    s = Chem.MolFromSmiles(Chem.MolToSmiles(x, isomericSmiles=True)
    ...
    while True:
        ...
        if flag:
            ...
            for b in mol.GetAtomWithIdx(idx).GetBonds():
                queue.append((b.GetIdx(), int(b.GetBondType()), b.
                    GetBeginAtomIdx(), b.GetEndAtomIdx()))
...
def adj_to_smiles(adj, x, atomic_num_list):
    valid = [Chem.MolToSmiles(construct_mol(x_elem, adj_elem,
        atomic_num_list), isomericSmiles=True) for x_elem, adj_elem in zip(
        x, adj)]
    return valid
...
def check_validity(adj, x, atomic_num_list, gpu=-1, return_unique=True,
    ...
    if correct_validity:
        ...
        for x_elem, adj_elem in zip(x, adj):
            ...
            vcmol = valid_mol_can_with_seg(cmol, largest_connected_comp=
    ...
    if debug:
        print('valid molecules: {}/{}'.format(len(valid), tuple(adj.shape)[0]))
        for i, mol in enumerate(valid):
            print('[{}] {}'.format(i, Chem.MolToSmiles(mol, isomericSmiles=
                False)))
    n_mols = tuple(x.shape)[0]
    valid_ratio = len(valid) / n_mols
    valid_smiles = [Chem.MolToSmiles(mol, isomericSmiles=False) for mol in
        valid]
    ...
    if debug:
        print('valid: {:.3f}%, unique: {:.3f}%, abs unique: {:.3f}%'.format
...
def check_novelty(gen_smiles, train_smiles, n_generated_mols):
    ...
    print('novelty: {:.3f}%, abs novelty: {:.3f}%'.format(novel_ratio,
        abs_novel_ratio))
