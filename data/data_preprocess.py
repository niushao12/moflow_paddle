...
def parse():
    ...
    parser.add_argument('--data_name', type=str, default='qm9', choices=[
        'qm9', 'zinc250k'], help='dataset to be downloaded')
    parser.add_argument('--data_type', type=str, default='relgcn', choices=
        ['gcn', 'relgcn'])
...
if data_name == 'qm9':
    ...
    raise ValueError('[ERROR] Unexpected value data_name={}'.format(data_name))
NumpyTupleDataset.save(os.path.join(data_dir, '{}_{}_kekulized_ggnp.npz'.
    format(data_name, data_type)), dataset)
print('Total time:', time.strftime('%H:%M:%S', time.gmtime(time.time() -
    start_time)))
