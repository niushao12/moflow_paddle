...
class NumpyTupleDataset(paddle.io.Dataset):
    ...
    def __init__(self, datasets, transform=None):
        ...
        for i, dataset in enumerate(datasets):
            if len(dataset) != length:
                raise ValueError('dataset of the index {} has a wrong length'
                    .format(i))
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, (slice, list, np.ndarray)):
            ...
            batches = [tuple([batch[i] for batch in batches]) for i in
                range(length)]
        else:
    ...
    def save(cls, filepath, numpy_tuple_dataset):
        ...
        if not isinstance(numpy_tuple_dataset, NumpyTupleDataset):
            raise TypeError(
                'numpy_tuple_dataset is not instance of NumpyTupleDataset, got {}'
