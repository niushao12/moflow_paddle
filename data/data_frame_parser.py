...
class DataFrameParser(object):
    ...
    def __init__(self, preprocessor, labels=None, smiles_col='smiles',
        postprocess_label=None, postprocess_fn=None, logger=None):
    ...
    def parse(self, df, return_smiles=False, target_index=None,
        ...
        if isinstance(pp, GGNNPreprocessor):
            ...
            total_count = tuple(df.shape)[0]
            fail_count = 0
            success_count = 0
            for row in tqdm(df.itertuples(index=False), total=tuple(df.
                shape)[0]):
                ...
                try:
                    ...
                    logger.warning('parse(), type: {}, {}'.format(type(e).
                        __name__, e.args))
        ...
        if isinstance(result, (tuple, list)):
            ...
            if self.postprocess_fn is not None:
                result = self.postprocess_fn(result)
            dataset = NumpyTupleDataset([result])
        return {'dataset': dataset, 'smiles': smileses, 'is_successful':
            is_successful}
