...
def construct_atomic_number_array(mol, out_size=-1):
    ...
    if out_size < 0:
        ...
        raise ValueError(
            '`out_size` (={}) must be negative or larger than or equal to the number of atoms in the input molecules (={}).'
...
def construct_adj_matrix(mol, out_size=-1, self_connection=True):
    ...
    if s0 != s1:
        raise ValueError(
            'The adjacent matrix of the input moleculehas an invalid shape: ({}, {}). It must be square.'
            .format(s0, s1))
    if self_connection:
...
def construct_discrete_edge_matrix(mol, out_size=-1):
    ...
    bond_type_to_channel = {Chem.BondType.SINGLE: 0, Chem.BondType.DOUBLE: 
        1, Chem.BondType.TRIPLE: 2, Chem.BondType.AROMATIC: 3}
